...
export default {
    //===================================================================================
    // 사용자 맛방 전체조회
    allRoom: async(req: Request, res: Response) => {
        ...
        try {
            ...
            const result = arrTheRoom.map((room, idx: number) => ({
                roomId: room?._id,
    ...
    //===================================================================================
    //맛방 detail - 방 정보
    detailRoomInfo: async(req: Request, res: Response) => {
    ...
    //===================================================================================
    //맛방 detail - 멤버 리스트
    detailRoomMember: async(req: Request, res: Response) => {
        const { roomId } = req.params;
        const { userId } = res.locals.user;
    ...
    //===================================================================================
    // 맛방 detail - 맛집 리스트
    detailRoomStoreList: async(req: Request, res: Response) => {
        const { roomId } = req.params;
        ...
        try {
            if (existRoom) {
                ...
                for (let i = 0; i < storeId.length; i++) {
                    const storeInfo: any = await Store.findById(storeId[i]).exec();
    ...
    writeRoom: async(req: Request, res: Response) => {
        ...
        try {
            ...
            await UsersRoom.findOneAndUpdate({ userId: user.userId }, { $push: { roomSeq: createdRoom._id as unknown as string } }, { upsert: true });

            if (!!guestId) {
                for (let i = 0; i < guestId.length; i++) {
                    await UsersRoom.findOneAndUpdate({ userId: guestId[i] }, { $push: { roomSeq: createdRoom._id as unknown as string} }, { upsert: true });
                }
            }
            return res
                .status(200)
    ...
    //===================================================================================
    // 맛방 초대 (공유하기)
    inviteRoom: async(req: Request, res: Response) => {
        const { userId } = res.locals.user;
    ...
    //===================================================================================
    // 맛방에서 게스트 강퇴
    kickRoom: async(req: Request, res: Response) => {
        const { userId } = res.locals.user;
        const { roomId } = req.params;
        const { guestId } = req.body;
    ...
    //===================================================================================
    // 맛방 정보 수정
    rewriteRoom: async(req: Request, res: Response) => {
        const roomNameCheck = Joi.object({
            roomName: Joi.string().required().max(8).messages({
                'string.empty': '{{#label}}를 채워주세요.',
                'string.max': '{{#label}}는 최대 8글자입니다.',
        ...
        try {
            ...
            if (findRoom) {
                for (let i = 0; i < findRoom.length; i++) {
                    await Savelist.deleteOne(findRoom[i] as unknown as object);
                }
            }
    ...
    //==============================================================
    // 맛방 나가기
    exitRoom: async(req: Request, res: Response) => {
        const { userId } = res.locals.user;
        const { roomId } = req.params;
    ...
    //==============================================================
    //맛방 순서 변경
    setSequenceRoom: async(req: Request, res: Response) => {
    ...
    //==============================================================
    // 맛방에 맛집 저장
    saveStore: async(req: Request, res: Response) => {
        const { roomId } = req.params;
        const { userId } = res.locals.user;
    ...
    //==============================================================
    // 룸코드 생성 코드
    findRoomCode: async(req: Request, res: Response) => {
        const { roomId } = req.params;
    ...
    //==============================================================
    //룸코드로 검색
    searchRoomCode: async(req: Request, res: Response) => {
    ...
    //==============================================================
    //룸코드로 맛방입장
    roomCode: async(req: Request, res: Response) => {
        const { userId } = res.locals.user;
        const { roomCode } = req.body;
        ...
        try {
            ...
            if (findRoom) {
                ...
                // 유저가 guest에 없다면 방 입장
                await Room.findByIdAndUpdate({ _id: findRoom._id }, {
                    $push: { guestId: userId },
                });

                await UsersRoom.findOneAndUpdate({ userId: userId }, {
                    $push: { roomSeq: findRoom._id as unknown as string}}
                    ,{ upsert : true },
                );
                return res.status(200).send({ result: 'success' });
            } else {
